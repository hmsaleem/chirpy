# *Harvest* - a command line interface for quickly collecting and parsing Twitter data.

Harvest is a command line tool for swift and streamlined collection of Twitter data. The toolset is capable of collecting already posted tweets, actively tracking the live Twitter feed for specific keywords and retrieving user history along with additional resources for parsing captured data.
...
To install `harvest` on your system, simply run `sudo python setup.py install`. 

***

## Configuration

On installation, harvest creates a directory `.harvest` in user home. The directory houses the file `harvest.config` that saves the paths for log files and profile files. By default, they are stored in `.harvest/logs` and `.harvest/profiles` respectively. A root path can also be specified to save the captured files in a particular directory.

***

## Synopsis

`harvest [options] flags`
...
```
harvest search –k \#hashtag –o output_dir –f output_file -n num
```
...
Example: `harvest search -k hello -o testhello -f hello.txt -n 800`
...
```
user:~$ harvest search -k hello -o testhello -f hello.txt -n 800
...
```
harvest user –user_name –o output_dir 
```
...
Example: `harvest user -u saleemq90 -o testhello`
...
```
user:~$ harvest user -u saleemq90 -o testhello
...
## Additional searches / streams

Presently we have 5 accounts, so we can run 5 searches and streams concurrently.
To run concurrent processes, make new tabs `CMD+t` and login into the server through ssh. You can run the processes separately in each tab.
Find top hashtag in a file
To find the top hashtag in an input file 
